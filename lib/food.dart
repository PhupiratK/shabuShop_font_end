...
class _CartPageState extends State<CartPage> {
  Future<void> submitOrder(BuildContext context) async {
    ...
    final tableId = 1;
    final totalPrice = 229;
    final drinkId = 0;
    final dessertId = 0;

    final orderData = {
      'table_id': tableId,
      'food_id': widget.cart.items.map((foodItem) => foodItem.id).toList(),
      'amount': widget.cart.items.map((foodItem) => foodItem.quantity).toList(),
      'total': totalPrice,
      'drink_id': drinkId,
    ...
    final response = await http.post(
      Uri.parse('http://localhost:3001/order'),
